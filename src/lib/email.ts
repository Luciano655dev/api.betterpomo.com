...
export function renderBrandedEmail(opts: BrandedEmailOptions): string {
  ...
  const bullets = opts.bullets?.length
    ? `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:6px 0 22px;">${opts.bullets
        .map((bullet) => `
          <tr>
            <td width="28" valign="top" style="padding:7px 0;color:#65a30d;font-size:18px;line-height:1.45;">●</td>
  ...
  const notice = opts.notice
    ? `<div style="margin:4px 0 22px;padding:15px 17px;border-left:4px solid #84cc16;border-radius:10px;background:#f7fee7;color:#365314;font-size:14px;line-height:1.6;">${withLineBreaks(opts.notice)}</div>`
    : "";
  ...
  const secondaryAction = opts.secondaryAction
    ? `<p style="margin:0 0 22px;color:#71717a;font-size:14px;line-height:1.6;"><a href="${escapeHtml(opts.secondaryAction.url)}" style="color:#3f6212;font-weight:700;text-decoration:underline;">${escapeHtml(opts.secondaryAction.label)}</a></p>`
    : "";
  ...
  <head>
    ...
    <style>
      ...
      @media (prefers-color-scheme: dark) {
        ...
        .bp-subtle { background:#252529 !important; border-color:#3f3f46 !important; }
      }
    </style>
  </head>
        ...
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:600px;">
          <tr>
            <td style="padding:0 8px 16px;">
              <a href="${APP_URL}" class="bp-brand" style="color:#18181b;text-decoration:none;font-size:18px;font-weight:800;letter-spacing:-.02em;"><span style="display:inline-block;margin-right:8px;width:12px;height:12px;border-radius:50%;background:#84cc16;"></span>BetterPomo</a>
            </td>
          </tr>
          <tr><td class="bp-card" style="padding:42px 44px;border:1px solid #e4e4e7;border-radius:24px;background:#ffffff;box-shadow:0 12px 35px rgba(24,24,27,.06);">
            ${opts.eyebrow ? `<p style="margin:0 0 12px;color:#65a30d;font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.16em;">${escapeHtml(opts.eyebrow)}</p>` : ""}
            ...
            ${paragraphs}${details}${bullets}${notice}${action}${secondaryAction}${signoff}
          </td></tr>
