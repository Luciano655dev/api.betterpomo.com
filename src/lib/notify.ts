...
function copyFor(opts: NotifyOptions): { category: PushCategory; payload: PushPayload } {
  ...
  const actorEmoji = typeof opts.metadata?.emoji === "string" ? opts.metadata.emoji : "🍅";
  ...
  switch (opts.type) {
    case "friend_request":
      return {
        category: "friends",
        payload: { title: "🤝 New friend request", body: `${actorEmoji} ${displayName} wants to focus with you.`, data },
      };
    case "friend_accept":
      return {
        category: "friends",
        payload: { title: "🎉 You're focus friends!", body: `${actorEmoji} ${displayName} accepted your friend request.`, data },
      };
    case "session_invite":
      return {
        category: "sessions",
        payload: { title: "⏱️ Ready to focus?", body: `${actorEmoji} ${displayName} invited you to ${name}.`, data },
      };
    case "group_add":
      return {
        category: "messages",
    ...
    case "trial_ending":
      return {
        ...
        payload: {
          ...
          body: "Your subscription starts in 2 days. You can manage it anytime in Settings.",
          data,
        },
      };
  }
